...
void ParallelTest(const std::function<bool(std::size_t)> &f,
                  std::size_t threads_cnt = 2) {
  std::vector<std::thread> threads;
  std::vector<std::atomic<bool>> check(threads_cnt);
  for (std::size_t t = 0; t < threads_cnt; ++t) {
    threads.emplace_back([&check, t, &f]() { check[t] = f(t); });
  }
  for (std::size_t t = 0; t < threads_cnt; ++t) {
    threads[t].join();
  }
...
TEST(MultiThread, Read) {
  KVStorage storage;
  storage.set("a", "2", 0);
  ParallelTest([&storage](std::size_t) { return storage.get("a") == "2"; }, 5);
}
